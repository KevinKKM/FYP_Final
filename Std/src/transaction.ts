...
const COIN_GENESIS: number = 50;
const COIN_AUTHORIZE: number = 1;
...
const validateCoinbaseTx = (transaction: Transaction, blockIndex: number): boolean => {
    ...
    if (getTransactionId(transaction) !== transaction.id) {
        ...
        console.log('valid coinbase tx id: ' + getTransactionId(transaction));
        return false;
    }
    ...
    if (transaction.txOuts[0].amount !== COIN_GENESIS && transaction.txOuts[0].amount !== COIN_AUTHORIZE) {
...
const getCoinbaseTransaction = (address: string, blockIndex: number): Transaction => {
    ...
    t.txIns = [txIn];
    t.txOuts = [new TxOut(address, COIN_AUTHORIZE)];
